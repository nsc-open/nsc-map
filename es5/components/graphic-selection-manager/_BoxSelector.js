...
class BoxSelector extends BaseSelector {
  constructor(args) {
    ...
    _defineProperty(this, "_mapMouseDownHandler", e => {
      if (!this._ready) {
        return;
      }
    ...
    _defineProperty(this, "_mapMouseMoveHandler", e => {
      if (this._startPoint) {
        e.stopPropagation();
        const {
          Extent,
        ...
        const ext = new Extent({
          xmin: Math.min(this._startPoint.x, mapPoint.x),
          ymin: Math.min(this._startPoint.y, mapPoint.y),
          xmax: Math.max(this._startPoint.x, mapPoint.x),
          ymax: Math.max(this._startPoint.y, mapPoint.y),
          spatialReference: {
            wkid: 102100
          }
        });
    ...
    _defineProperty(this, "_mapMouseUpHandler", e => {
      if (this._boxGraphic) {
        e.stopPropagation();
